...
Installation writes program files under the current user's LocalAppData Programs folder and can write a current-user autostart entry. English mode changes the taskbar appearance and standard Arrow/Hand cursor roles in the Windows session. Normal stop/uninstall attempts restoration; abrupt termination and third-party customization can affect restoration.
