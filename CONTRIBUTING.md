...
Run `python tools/release.py verify-source`, `python tools/release.py verify-installer`, `python -m unittest discover -s tools -p "test_*.py"`, and `go test ./...` in `source/`. Run Windows vet/build as described in the README. Packaging changes must preserve the reviewed source manifest unless a deliberate runtime update is being reviewed.
